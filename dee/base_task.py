...
from dee.utils import default_dump_pkl, default_dump_json, default_load_pkl
    ...
    def _load_data(self, load_example_func, convert_to_feature_func, convert_to_dataset_func,
        ...
        if load_dev:
            ...
            if self.setting.with_pkl_file:
                self.logging('Load pickle file for dev')
                pkl_fp = os.path.join(self.setting.data_dir, self.setting.dev_pkl_name)
                self.dev_pkl = default_load_pkl(pkl_fp)
        ...
        if load_test:
            ...
            if self.setting.with_pkl_file:
                self.logging('Load pickle file for test')
                pkl_fp = os.path.join(self.setting.data_dir, self.setting.test_pkl_name)
                self.test_pkl = default_load_pkl(pkl_fp)
        else:
    ...
    def base_train(self, get_loss_func, kwargs_dict1={},
        ...
        for epoch_idx in trange(base_epoch_idx, int(self.setting.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in step_batch_iter:
                ...
                if step % self.setting.logging_steps == 0:
                    if step == 0:
                        self.logging(f"the initial loss is {loss_scalar}")
                    else:
                        avg_loss = sum(batch_loss_list[last_logging_step: step]) / self.setting.logging_steps
                        self.logging(f"the average loss of recent {self.setting.logging_steps} is {avg_loss}")
                        last_logging_step = step

            if epoch_eval_func is not None:
                epoch_eval_func(self, epoch_idx + 1, **kwargs_dict2)
            curr_epoch_avg_loss = sum(batch_loss_list) / len(batch_loss_list)
            self.logging(f"average loss of the current epoch is {curr_epoch_avg_loss}")

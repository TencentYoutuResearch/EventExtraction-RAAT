...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ...
        ('dev_pkl_name', 'dev.pkl'),
        ('test_pkl_name', 'test.pkl'),
        ...
        ('cpt_file_name', 'ReDEE'),  # decide the identity of checkpoints, evaluation results, etc.
        ('model_type', 'ReDEE'),  # decide the model class used
        ...
        ('with_post_process', False),  # whether to fuse multiple events into single.
        ('with_pkl_file', False),  # whether to load pkl file for dev and test files.
        ('use_re', True),
        ...
        ("logging_steps", 1),
        ("re_loss_ratio", 1.0),
        ("raat", False),  # whether to substitute vanilla transformer-2 with RAAT module.
...
class DEETask(BasePytorchTask):
    ...
    def __init__(self, dee_setting, load_train=True, load_dev=True, load_test=True,
        ...
        # get entity and event label name (BIO format)
        self.entity_label_list = DEEExample.get_entity_label_list()

        # get entity and event label name (BIEOS format)
        ...
        # initialize relation extraction module.

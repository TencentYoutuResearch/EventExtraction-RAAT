...
from dee.dee_metric import measure_event_table_filling
from dee.event_type import event_type2event_class, BaseEvent, event_type_fields_list, common_fields
from dee.ner_task import NERExample, NERFeatureConverter
from dee.utils import default_load_json, default_dump_json, default_dump_pkl, default_load_pkl
...
class DEEExampleLoader(object):
    ...
    def convert_dict_to_example(self, annguid, detail_align_info, only_inference=False):
        ...
        dee_example = DEEExample(annguid, detail_align_info, only_inference=self.only_inference)

        return dee_example

    def __call__(self, dataset_json_path, sentence_input_dict=None):
        """Either to provide dataset_json_path, or provide sentence_input_dict for model inference. """
        total_dee_examples = []
        if sentence_input_dict:
            dee_example = self.convert_dict_to_example(0, sentence_input_dict)
            return [dee_example]
...
def measure_dee_prediction(event_type_fields_pairs, features, event_decode_results,
                           dump_json_path=None, with_post_process=False, vocab_list=None):
    ...
    for term in event_decode_results:
        ...
        if with_post_process:
            pred_record_mat = merge_multi_events(pred_record_mat, vocab_list)
            gold_record_mat = merge_multi_events(gold_record_mat, vocab_list)
...
def measure_dee_prediction_with_pickle_input(event_type_fields_pairs, pickle_data, event_decode_results,
                                             dump_json_path=None, with_post_process=False, vocab_list=None):
    pred_record_mat_list = []
    gold_record_mat_list = [data[2] for data in pickle_data]
    for term in event_decode_results:
        ex_idx, pred_event_type_labels, pred_record_mat = term[:3]
        pred_record_mat = [
            [
...
def print_total_eval_info(data_span_type2model_str2epoch_res_list,
                          metric_type='micro',
                          span_type='pred_span',
                          model_strs=('DCFEE-O', 'DCFEE-M', 'GreedyDec', 'ReDEE'),
                          target_set='test'):
...
def print_single_vs_multi_performance(mstr_bepoch_list, base_dir, features,
                                      ...
                                      with_post_process=False, vocab_list=None):
    ...
    for model_str, _ in mstr_bepoch_list:
        ...
        single_eval_res = measure_dee_prediction(
            ...
            with_post_process=with_post_process, vocab_list=vocab_list
        )
...
def merge_multi_events(event_idx2obj_idx2field_idx2token_tup, vocab_list, delimiter='/', process_for_display=False):
    """merge multiple events to one if they have all roles in common except one. if the distinct role entities
    have the contained relation, keep the longest one. Otherwise, keep all of them, with '/' as delimiter. """
    filter_event_idx2obj_idx2field_idx2token_tup = []
    for multi_events, event_type_fields in zip(event_idx2obj_idx2field_idx2token_tup, event_type_fields_list):
        if multi_events is None:
            filter_event_idx2obj_idx2field_idx2token_tup.append(multi_events)
        else:
            multi_events = copy.deepcopy(multi_events)
            if len(multi_events) == 1:
                filter_event_idx2obj_idx2field_idx2token_tup.append(multi_events)
            else:
                # Get event fields to judge when to get PersonName and P
                event_roles = event_type_fields[1]
                curr_type_event = [multi_events[0]]
                multi_events = multi_events[1:]
                while len(multi_events) > 0:
                    # record whether the current event has been merged to the existed ones.
                    has_merge = False
                    curr_event = multi_events[0]
                    multi_events = multi_events[1:]
                    for event_id in range(len(curr_type_event)):
                        # count the number of distinct event roles.
                        distinct_event_roles = set()
                        num_distinct_role = 0
                        distinct_role_idx = []
                        for i, (role_name, role_1, role_2) in\
                                enumerate(zip(event_roles, curr_event, curr_type_event[event_id])):
                            if isinstance(role_2, list) and role_1:
                                if role_1 not in role_2:
                                    num_distinct_role += 1
                                    distinct_role_idx.append(i)
                                    distinct_event_roles.add(role_name)
                            elif role_1 != role_2:
                                num_distinct_role += 1
                                distinct_role_idx.append(i)
                                distinct_event_roles.add(role_name)
                        if process_for_display:
                            # In display mode, for two events, there are two different situations:(The first strategy
                            # is not used right now)
                            # 1. if all entities are same except for PersonName and Positions, then merge them;
                            # 2. If all entities are same except for PersonName, then not merge them, since PersonName&
                            # Position mapping has problem.

                            # if num_distinct_role == 2 and distinct_event_roles == {"PersonName", "Position"}:
                            #     for role_idx in distinct_role_idx:
                            #         if isinstance(curr_type_event[event_id][role_idx], tuple):
                            #             curr_type_event[event_id][role_idx] = \
                            #                 [curr_type_event[event_id][role_idx], curr_event[role_idx]]
                            #         elif curr_type_event[event_id][role_idx] is None:
                            #             curr_type_event[event_id][role_idx] = [None, curr_event[role_idx]]
                            #         else:
                            #             curr_type_event[event_id][role_idx].append(curr_event[role_idx])
                            #     has_merge = True
                            #     break

                            if num_distinct_role == 1 and (distinct_event_roles == {"PersonName"} or distinct_event_roles == {"Event"}):
                                role_idx = distinct_role_idx[0]
                                # If one person_name is part of the other person_name, retain the longer one.
                                # if isinstance(curr_type_event[event_id][role_idx], list):
                                #     for i, tup in enumerate(curr_type_event[event_id][role_idx]):
                                #         if set(tup).issubset(curr_event[role_idx]) or set(curr_event[role_idx]).issubset(tup):
                                #             curr_type_event[event_id][role_idx][i] = tup\
                                #                 if len(tup) > len(curr_event[role_idx]) else curr_event[role_idx]
                                #             has_merge = True
                                #             break
                                # else:
                                #     if set(curr_type_event[event_id][role_idx]).issubset(curr_event[role_idx]) or\
                                #             set(curr_event[role_idx]).issubset(curr_type_event[event_id][role_idx]):
                                #         curr_type_event[event_id][role_idx] = curr_type_event[event_id][role_idx]\
                                #             if len(curr_type_event[event_id][role_idx]) > len(curr_event[role_idx])\
                                #             else curr_event[role_idx]
                                #         has_merge = True
                                #         break
                                if curr_type_event[event_id][role_idx] is None:
                                    curr_type_event[event_id][role_idx] = curr_event[role_idx]
                                    has_merge = True
                                    break
                                elif curr_event[role_idx] is None:
                                    has_merge = True
                                    break
                                elif set(curr_type_event[event_id][role_idx]).issubset(curr_event[role_idx]) or \
                                        set(curr_event[role_idx]).issubset(curr_type_event[event_id][role_idx]):
                                    curr_type_event[event_id][role_idx] = curr_type_event[event_id][role_idx] \
                                        if len(curr_type_event[event_id][role_idx]) > len(curr_event[role_idx]) \
                                        else curr_event[role_idx]
                            ...
                            elif num_distinct_role == 1:
                                role_idx = distinct_role_idx[0]
                                if curr_type_event[event_id][role_idx] is None:
                                    curr_type_event[event_id][role_idx] = curr_event[role_idx]
                                elif curr_event[role_idx] is None:
                                    pass
                                elif isinstance(curr_type_event[event_id][role_idx], tuple):
                                    curr_type_event[event_id][role_idx] = \
                                        [curr_type_event[event_id][role_idx], curr_event[role_idx]]
                                else:
                                    curr_type_event[event_id][role_idx].append(curr_event[role_idx])
                            ...
                            elif num_distinct_role == 0:
                                has_merge = True
                                break

                        elif num_distinct_role == 1:
                            # if one event role has multiple candidates, put them in the list.
                            role_idx = distinct_role_idx[0]
                            if isinstance(curr_type_event[event_id][role_idx], tuple):
                                curr_type_event[event_id][role_idx] = \
                                    [curr_type_event[event_id][role_idx], curr_event[role_idx]]
                            ...
                            break
                    if not has_merge:
                        curr_type_event.append(curr_event)

                for event in curr_type_event:
                    # Process the event role with multiple entities.
                    # In display mode, if both PersonName and Position has multiple entities, do not merge them.
                    # if event_type_fields[0] in ["Meeting", "Activity"]:
                    #     person_id = event_roles.index("PersonName")
                    #     position_id = event_roles.index("Position")
                    #     if isinstance(event[person_id], list) and isinstance(event[position_id], list):
                    #         continue
                    for role_id, entity in enumerate(event):
                        if isinstance(entity, list):
                            event[role_id] = aggregate_multi_entities(entity, vocab_list, delimiter)
                filter_event_idx2obj_idx2field_idx2token_tup.append(curr_type_event)

    return filter_event_idx2obj_idx2field_idx2token_tup


def aggregate_multi_entities(entity_id_list, vocab_list, delimiter='/', unk_tok='[UNK]'):
    """aggregate multiple entities into one. The rules are as follow:
    1. if one entity is completely overlapped by another, remove it.
    2. concate entities with '/'
    """
    id_2_vocab = {i: token for i, token in enumerate(vocab_list)}
    vocab_2_id = {val: k for k, val in id_2_vocab.items()}
    unk_id = vocab_2_id[unk_tok]

    if id_2_vocab is None:
        raise ValueError("id_2_vocab should be assigned to the model.")
    filter_entity_id_list = []
    for e in entity_id_list:
        if e is not None:
            filter_entity_id_list.append(e)
    entity_list = ["".join([id_2_vocab[i] for i in entity_ids if i != unk_id]) for entity_ids in filter_entity_id_list]
    entity_list = sorted(entity_list, key=lambda x: len(x))
    filter_list = []
    for i in range(0, len(entity_list) - 1):
        ...
        for j in range(i + 1, len(entity_list)):
            if entity_list[i] in entity_list[j]:
                ...
                break
        if not overlap:
            filter_list.append(entity_list[i])
    filter_list.append(entity_list[-1])
    if len(filter_list) == 1:
        entity_id_tup = tuple([vocab_2_id[i] for i in filter_list[0]])
    else:
        # sort the list of entity to avoid order problem.
        filter_list = sorted(filter_list)
        delimiter_id = vocab_2_id[delimiter]
        entity_id_tup_list = [[vocab_2_id[i] for i in entity_str] + [delimiter_id]
                              for entity_str in filter_list]
        flatten_entity_id_tup_list = [item for sublist in entity_id_tup_list for item in sublist]
        flatten_entity_id_tup_list.pop()
        entity_id_tup = tuple(flatten_entity_id_tup_list)

    return entity_id_tup

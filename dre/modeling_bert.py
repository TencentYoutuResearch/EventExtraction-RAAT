...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, with_naive_feature):
        ...
        if self.with_naive_feature:
            self.ner_emb = nn.Embedding(30, config.hidden_size, padding_idx=0)  # 30 for ChiFinAnn, 61 for DuEE-fin
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, entity_structure):
        ...
        if entity_structure != 'none':
            ...
            self.abs_bias = nn.Parameter(torch.zeros(num_structural_dependencies, self.num_attention_heads))
    ...
    def forward(
        ...
        if self.entity_structure != 'none':
            ...
            attention_bias = torch.einsum("bnip,knpq,bnjq->kbnij", query_layer, self.bili, key_layer)
            attention_bias = (attention_bias + self.abs_bias[:, None, :, None, None]) * structure_mask
...
class BertForDocRED(BertPreTrainedModel):
    ...
    Examples::
        ...
        # generate ent_rep before dim reduction.
        if ent_mask.dtype != outputs.dtype:
            ent_mask = ent_mask.to(outputs.dtype)
        full_dim_ent_rep = torch.matmul(ent_mask, outputs)
        ...
        # Convert into cross_entropy loss
        loss_fct = CrossEntropyLoss(reduction="none")
        loss_all_ent_pair = loss_fct(logits.view(-1, self.num_labels), label.view(-1))
        loss_all_ent_pair = loss_all_ent_pair.view(-1, self.max_ent_cnt, self.max_ent_cnt)
        # loss_per_example = torch.sum(loss_all_ent_pair * label_mask, dim=[1, 2]) / torch.sum(label_mask, dim=[1, 2])
        loss_per_example = torch.sum(loss_all_ent_pair * label_mask, dim=[1, 2])
        loss = torch.mean(loss_per_example)

        # use customised loss fn to solve label imbalance problem.
        # loss = custom_loss_fn(logits, label, label_mask)
        ...
        # # loss_per_example = torch.sum(loss_all_ent_pair * label_mask, dim=[1, 2]) / torch.sum(label_mask, dim=[1, 2])
        # # loss = torch.mean(loss_per_example)
        # loss = torch.sum(loss_all_ent_pair * label_mask, dim=[1, 2])
        # loss = torch.mean(loss)
        # logits = torch.sigmoid(logits)

        return loss, logits, full_dim_ent_rep  # (loss), logits, entity vector representation.


def custom_loss_fn(logits, labels, label_mask):
    # [bz, max_ent_cnt, max_ent_cnt]
    assert(label_mask.dim() == labels.dim() == 3)
    # [bz, max_ent_cnt, max_ent_cnt, num_labels]
    assert(logits.dim() == 4)

    batch_size, max_ent_cnt, _, num_labels = logits.size()
    logits = logits.view(-1, max_ent_cnt * max_ent_cnt, num_labels)
    labels = labels.view(-1, max_ent_cnt * max_ent_cnt)
    label_mask = label_mask.view(-1, max_ent_cnt * max_ent_cnt)
    total_loss = None
    for i in range(batch_size):
        sample_logits, sample_labels, sample_label_mask = logits[i], labels[i], label_mask[i]

        sample_labels[sample_label_mask == False] = -1
        pos_logits = sample_logits[torch.arange(sample_labels.size()[0])[sample_labels > 0], sample_labels[sample_labels > 0]]
        neg_logits = sample_logits[torch.arange(sample_labels.size()[0])[sample_labels == 0], sample_labels[sample_labels == 0]]
        pos_loss = torch.log(1 + torch.sum(torch.exp(-pos_logits)))
        neg_loss = torch.log(1 + torch.sum(torch.exp(neg_logits)))

        # consider augmenting re loss by multiplying number of positive pair.
        # TODO: check validation.
        sample_loss = (pos_loss + neg_loss) * pos_logits.size()[0]
